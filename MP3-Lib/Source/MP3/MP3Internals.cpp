...
static unsigned const live_tabsel[2][3][16] = {
    {{32, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448,
      448},
...
static long const live_freqs[] = {44100, 48000, 32000, 22050, 24000,
...
MP3FrameParams::MP3FrameParams()
    : isMPEG2(0),
      samplingFreq(44100),
      frameSize(413),  // init params, in case we're used early
...
#define TRUNC_FAIRLY
static unsigned updateSideInfoSizes(
    MP3SideInfo& sideInfo, Boolean isMPEG2, unsigned char const* mainDataPtr,
    unsigned allowedNumBits, unsigned& part23Length0a,
    unsigned& part23Length0aTruncation, unsigned& part23Length0b,
    unsigned& part23Length0bTruncation, unsigned& part23Length1a,
    unsigned& part23Length1aTruncation, unsigned& part23Length1b,
...
#ifdef TRUNC_ONLY0
  if (p23L0 < allowedNumBits) allowedNumBits = p23L0;
#endif
#ifdef TRUNC_ONLY1
  if (p23L1 < allowedNumBits) allowedNumBits = p23L1;
#endif
...
Boolean GetADUInfoFromMP3Frame(unsigned char const* framePtr,
                               unsigned totFrameSize, unsigned& hdr,
                               unsigned& frameSize, MP3SideInfo& sideInfo,
...
static void getSideInfo1(MP3FrameParams& fr, MP3SideInfo& si, int stereo,
  ...
  for (gr = 0; gr < 2; gr++) {
    for (ch = 0; ch < stereo; ch++) {
      ...
      /* window-switching flag == 1 for block_Type != 0 .. and block-type == 0
       * -> win-sw-flag = 0 */
...
static void getSideInfo2(MP3FrameParams& fr, MP3SideInfo& si, int stereo,
                         int ms_stereo, long sfreq, int /*single*/) {
  int ch;
#if 0
   int powdiff = (single == 3) ? 4 : 0;
...
static void putSideInfo1(BitVector& bv, MP3SideInfo const& si,
                         Boolean isStereo) {
  int ch, gr, i;
  ...
  for (gr = 0; gr < 2; gr++) {
    for (ch = 0; ch < stereo; ch++) {
      ...
      if (gr_info.window_switching_flag) {
        ...
        for (i = 0; i < 2; i++) bv.putBits(gr_info.table_select[i], 5);
        for (i = 0; i < 3; i++) bv.putBits(gr_info.subblock_gain[i], 3);
      } else {
        for (i = 0; i < 3; i++) bv.putBits(gr_info.table_select[i], 5);
...
static void putSideInfo2(BitVector& bv, MP3SideInfo const& si,
                         Boolean isStereo) {
  int ch, i;
  int stereo = isStereo ? 2 : 1;
...
static unsigned MP3BitrateToBitrateIndex(unsigned bitrate /* in kbps */,
  ...
  for (unsigned i = 1; i < 15; ++i) {
    if (live_tabsel[isMPEG2][2][i] >= bitrate) return i;
  }
...
unsigned TranscodeMP3ADU(unsigned char const* fromPtr, unsigned fromSize,
                         unsigned toBitrate, unsigned char* toPtr,
                         unsigned toMaxSize,
                         unsigned& availableBytesForBackpointer) {
  ...
  unsigned numAduBits = updateSideInfoSizes(
      sideInfo, outFr.isMPEG2, fromPtr, 8 * desiredOutAduSize, part23Length0a,
      part23Length0aTruncation, part23Length0b, part23Length0bTruncation,
      part23Length1a, part23Length1aTruncation, part23Length1b,
      part23Length1bTruncation);
#ifdef DEBUG
  fprintf(
      stderr,
      "shrinkage %d->%d [(%d,%d),(%d,%d)] (trunc: [(%d,%d),(%d,%d)]) {%d}\n",
      inAduSize, (numAduBits + 7) / 8, part23Length0a, part23Length0b,
      part23Length1a, part23Length1b, part23Length0aTruncation,
      part23Length0bTruncation, part23Length1aTruncation,
      part23Length1bTruncation, maxOutAduSize);
#endif
  unsigned actualOutAduSize = (numAduBits + 7) / 8;

  // Give the new ADU an appropriate 'backpointer':
  assignADUBackpointer(outFr, actualOutAduSize, sideInfo,
                       availableBytesForBackpointer);
