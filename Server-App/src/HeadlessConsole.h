...
class HeadlessConsole {
 ...
 public:
  ...
  template <typename... Args>
  void AddTaggedMessage(std::string_view tag, std::string_view format,
                        Args&&... args) {
    std::string messageString =
        fmt::vformat(format, fmt::make_format_args(args...));
  ...
  template <typename... Args>
  void AddMessageWithColor(uint32_t color, std::string_view format,
                           Args&&... args) {
    std::string messageString =
        fmt::vformat(format, fmt::make_format_args(args...));
  ...
  template <typename... Args>
  void AddItalicMessageWithColor(uint32_t color, std::string_view format,
                                 Args&&... args) {
    std::string messageString =
        fmt::vformat(format, fmt::make_format_args(args...));
  ...
  template <typename... Args>
  void AddTaggedMessageWithColor(uint32_t color, std::string_view tag,
                                 std::string_view format, Args&&... args) {
    std::string messageString =
        fmt::vformat(format, fmt::make_format_args(args...));
    m_MessageHistory.push_back(
        MessageInfo(std::string(tag), messageString, color));
    std::cout << '[' << tag << "] " << messageString << std::endl;
  }
 ...
 private:
  struct MessageInfo {
    ...
    MessageInfo(const std::string& tag, const std::string& message,
                uint32_t color = 0xffffffff)
